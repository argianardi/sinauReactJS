...
## React-beautiful-dnd (Fitur drag and drop)

React Beautiful DND adalah sebuah library yang dirancang untuk memudahkan implementasi fitur drag-and-drop yang halus dan interaktif di aplikasi web menggunakan React. Dengan React Beautiful DND, developer dapat dengan mudah membuat UI yang mendukung drag dan drop elemen-elemen di berbagai bagian halaman, sehingga memberikan UX yang lebih baik. Pembahasan lebih lanjutnya bisa dilihat dalam dokumentasinya [disini](https://github.com/atlassian/react-beautiful-dnd)
...
- `<DragDropContext />` <br/>
  Tag `<DragDropContext />` bertindak sebagai control utama dalam implementasi drag-and-drop di aplikasi React. Kita harus membungkus bagian dari aplikasi yang akan dibuat fitur drag-and-drop dengan komponen ini. Fungsi utama dari tag ini adalah mengaktifkan dan mengelola seluruh proses drag-and-drop. Ketika pengguna menarik elemen yang diizinkan untuk di-drag, tag `<DragDropContext />` akan mendeteksi peristiwa ini dan memberikan informasi tentang elemen yang di-drag dan posisi awalnya. Anda juga harus menentukan properti `onDragEnd`, yang akan dipanggil ketika proses drag-and-drop selesai, untuk menangani perubahan posisi elemen yang di-drag.
- `<Droppable />` <br/>
  Tag `<Droppable />` adalah area yang dapat menerima elemen yang di-drop oleh user. Kita harus menempatkan elemen-elemen yang ingin di-drop di dalam tag ini. Ketika pengguna menarik elemen yang di-drag dan mengarahkannya ke area yang memiliki tag `<Droppable />`, area tersebut menjadi zona potensial untuk melepaskan elemen tersebut. Dengan menggunakan tag ini, kita dapat menentukan zona-zona yang berbeda dalam aplikasi kita yang menerima elemen yang di-drop. Setiap <Droppable /> memiliki properti droppableId yang harus unik dalam aplikasi dan digunakan untuk mengidentifikasi zona di mana elemen dapat di-drop.
- `<Draggable />`<br/>
...
Kita juga bisa menggunakan psudo class dari tailwind yaitu `print:`. Pseudo class ini digunakan untuk memanipulasi style untuk komponen saat sedang di-print. Berikut contoh penggunaannya:
...
ApexCharts adalah sebuah library JavaScript yang digunakan untuk membuat berbagai jenis grafik interaktif dalam aplikasi web. Library ini memanfaatkan SVG untuk merender grafik, serta didukung oleh berbagai fitur modern seperti animasi, responsif, dan interaktivitas. Apex Charts dibahas lebih lengkap di [dokumetasi resminya](https://apexcharts.com/docs/options/responsive/)

### Fitur Utama ApexCharts

- Tampilan Interaktif <br/> ApexCharts memungkinkan pengguna berinteraksi dengan grafik melalui fitur zoom, hover, klik, dan lainnya.
- Berbagai Jenis Grafik <br/> Library ini mendukung berbagai jenis grafik, termasuk grafik garis, grafik batang, grafik area, grafik lingkaran, grafik donat, dan lainnya.
- Responsif <br/> Grafik yang dibuat dengan ApexCharts dapat menyesuaikan diri dengan ukuran layar perangkat, sehingga tetap terlihat baik pada perangkat desktop maupun mobile.
- Animasi <br/> Grafik yang dibuat dengan ApexCharts mendukung animasi yang memberikan efek visual yang menarik.
- Tooltip Kustom <br/> Anda dapat menyesuaikan tooltip untuk menampilkan informasi tambahan saat pengguna mengarahkan kursor ke data grafik.
- Tema Bawaan <br/> ApexCharts memiliki beberapa tema bawaan yang dapat Anda pilih sesuai dengan tampilan aplikasi Anda.
- Customizable <br/> Library ini memberikan banyak option konfigurasi untuk mengubah tampilan, tata letak, dan perilaku grafik sesuai dengan kebutuhan Anda.

### Cara Menggunakan ApexCharts

Untuk menggunakan ApexCharts, kita perlu mengimpor library tersebut ke proyek kita dan kemudian menginisialisasi grafik dengan memberikan option dan data yang diperlukan. Berikut langkah-langkah umum dalam menggunakan ApexCharts:

- Instalasi <br/> kita dapat memasukkan library ApexCharts ke dalam proyek kita dengan menambahkan script atau melalui package manager seperti npm atau yarn.
- Konfigurasi Option <br/> kita dapat menyesuaikan tampilan dan perilaku grafik melalui berbagai option konfigurasi seperti tipe grafik, tema, axis, warna, dan lainnya.
- Menggunakan Data <br/> kita perlu memberikan data yang akan ditampilkan dalam grafik, misalnya data nilai untuk sumbu x dan sumbu y pada grafik garis.
- Menggambar Grafik <br/> Setelah menginisialisasi grafik dan menyediakan data, kita dapat menggunakan fungsi render untuk menggambar grafik di elemen yang ditentukan.

### Implementasi Apex Charts

Kita coba implementasikan apex charts ke dalam sebuah project, source codenya bisa dilihat [di sini](https://github.com/argianardi/sinauApexCharts) dan demonya juga bisa dilihat [di sini](https://opencharts.vercel.app/)

Berikut code contoh konfigurasi untuk membuat chart area menggunkan apex charts
...
[source code](https://github.com/argianardi/sinauApexCharts/blob/main/src/pages/OpenChart1.jsx)

Prilaku Chart ditentukan oleh konfigurasi di bagian options. Kita bahas satu - satu:
